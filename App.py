"""
Streamlit wrapper for Transparent batch master for AI mixes
Save this file as `app.py` and run: `streamlit run app.py`

Features:
- Upload one or more WAV files or point to a local folder (when running locally).
- Adjust a few mastering parameters from the sidebar.
- Shows live progress and per-file text logs.
- Provides download buttons for mastered files.

Notes:
- This app re-uses the original mastering functions almost verbatim (biquads, limiter, loudness measurement).
- Processing is CPU-heavy. On local machines you can choose number of workers.
- Dependencies: streamlit, numpy, soundfile, pyloudnorm
...
# ---- bring in the original mastering code (constants + functions) ----
# For brevity we import the bulk of the original script here. The functions
# are adapted to be file-path-agnostic and to run in a worker process.
...
# CPU friendliness (helpful when running on multi-core)
...
def compress_parallel(x, sr, thr_db=COMP_THRESH_DB, ratio=COMP_RATIO, knee_db=COMP_KNEE_DB,
    ...
    gr_db = compressor_gain_curve(level_db, thr_db, ratio, knee_db)
...
# ---- per-file processing (same as original, writing out _master.wav) ----

def process_file(path, LUFS_MIN_local=LUFS_MIN, LUFS_MAX_local=LUFS_MAX, TP_CEILING_DBTP_local=TP_CEILING_DBTP,
                 OS_FACTOR_local=OS_FACTOR):
    try:
        info = sf.info(path)
        ...
        tp_in = lin_to_db(truepeak_lin(x, L=OS_FACTOR_local))
        ...
        tp_pre = lin_to_db(truepeak_lin(y, L=OS_FACTOR_local))
        headroom = TP_CEILING_DBTP_local - tp_pre
        allowed_gain = headroom + GR_MAX_DB

        gain_to_top = LUFS_MAX_local - l_pre
        G = min(gain_to_top, allowed_gain)

        desired_L = l_pre + G
        if desired_L < LUFS_MIN_local:
            G = min(LUFS_MIN_local - l_pre, allowed_gain + 1.0)
            desired_L = l_pre + G

        y = y * db_to_lin(G)
        y = limit_truepeak_strict(y, tp_limit_db=TP_CEILING_DBTP_local, base_ceiling_db=TP_CEILING_DBTP_local)

        attempts = 0
        while attempts < 2:
            out_lufs = measure_lufs(y, sr)
            target_delta = 0.0
            if out_lufs > LUFS_MAX_local + GAIN_TOL_LU:
                target_delta = LUFS_MAX_local - out_lufs
            elif out_lufs < LUFS_MIN_local - 0.2:
                target_delta = min(LUFS_MIN_local - out_lufs, 0.8)
            if abs(target_delta) <= GAIN_TOL_LU:
                break
            y = limit_truepeak_strict(y * db_to_lin(target_delta),
                                      tp_limit_db=TP_CEILING_DBTP_local,
                                      base_ceiling_db=TP_CEILING_DBTP_local)
            attempts += 1

        y = np.clip(y, -1.0, 1.0)

        out_sp_db = lin_to_db(np.max(np.abs(y)))
        out_tp_db = lin_to_db(truepeak_lin(y, L=OS_FACTOR_local))
        out_lufs  = measure_lufs(y, sr)
        ...
        return (
            ...
            f"     Loudness: in {l_in:+.2f} LUFS → out {out_lufs:+.2f} LUFS (band {LUFS_MIN_local:+.1f}…{LUFS_MAX_local:+.1f})\n"
            f"     Peaks: in TP {tp_in:6.2f} dBTP → out TP {out_tp_db:6.2f} dBTP (ceil {TP_CEILING_DBTP_local:.1f}, {OS_FACTOR_local}× OS) | SP {out_sp_db:6.2f} dBFS"
        )
...
def main_streamlit():
    st.set_page_config(page_title='Transparent Batch Master', layout='wide')
    st.title('Transparent Batch Master — Streamlit')

    st.write("Upload one or more WAV files below. Files will be mastered using intellegent mastering AI")

    uploaded = st.file_uploader('Upload WAV files', type=['wav','WAV'], accept_multiple_files=True)

    if not uploaded:
        st.info('Waiting for WAV uploads...')
        return

    # Save uploads to a temporary directory
    tmpdir = tempfile.mkdtemp(prefix='st_master_')
    files = []
    for f in uploaded:
        safe = os.path.join(tmpdir, f.name)
    ...
    st.info(f'Found {len(files)} WAV file(s). Mastering with defaults: LUFS {LUFS_MIN}…{LUFS_MAX}, TP≤{TP_CEILING_DBTP} dBTP, {OS_FACTOR}× OS.')

    progress = st.progress(0)
    

    # Fixed default worker count (keeps behavior simple)
    workers = 4

    def worker_job(path):
        # Call process_file with defaults defined at top of the script
        return process_file(path)

    with ThreadPoolExecutor(max_workers=workers) as ex:
        futures = {ex.submit(worker_job, p): p for p in files}
        total = len(futures)
        done = 0
    ...
    # Provide downloads for produced files
    out_files = [p[:-4] + '_master.wav' for p in files if os.path.exists(p[:-4] + '_master.wav')]
    if out_files:
        st.header('Downloads')
        for of in out_files:
            with open(of, 'rb') as fh:
                data = fh.read()
            basename = os.path.basename(of)
            st.download_button(f'Download {basename}', data, file_name=basename, mime='audio/wav')
    else:
        st.warning('No output files were created — check logs above.')
